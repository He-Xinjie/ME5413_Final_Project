...
class BoxCandidate:
    ...
    def __init__(self, map_x, map_y, map_z, digit, scan_id=0, phase='lower'):
        self.x = map_x
        self.y = map_y
        self.z = map_z
        self.obs_count = 1
        ...
        # scan_id: which scan window this candidate was first discovered in.
        # Used to isolate per-room matching in upper phase without clearing
        # historical candidates (so markers persist for manual inspection).
...
class BoxDetector3D:
    def __init__(self):
        ...
        self.same_digit_radius = 2.0        # merge radius for SAME digit
        ...
        # Detection gate: stairway off, lower/upper on.
        self.detection_enabled = True
        # Phase + scan window. We never clear self.candidates anymore — instead
        # we tag each new candidate with the current scan_id, and the upper
        # room-matching only counts candidates whose scan_id matches the
        # latest reset. Lower-phase boxes stay in the list for marker display
        # but won't pollute upper room matching.
        self.current_phase = 'lower'
        self.current_scan_id = 0
        # Once we have a frozen lower-phase answer, never republish min_box.
        self.min_box_locked = False
        ...
        # Global costmap subscriber (for gap verification between same-digit candidates)
    ...
    def zone_cmd_callback(self, msg):
        ...
        if cmd == "lower":
            ...
            # Freeze the lower-phase min_box answer (latched) and start a new
            # scan window. Do NOT clear self.candidates — historical markers
            # persist for manual inspection.
            self.min_box_locked = True
            self.current_phase = 'upper'
            self.current_scan_id += 1
            self.detection_enabled = True
            rospy.loginfo(
                "[BoxDet3D] Upper phase: min_box LOCKED, scan_id=%d, "
                "kept %d historical candidates",
                self.current_scan_id, len(self.candidates))
    ...
    def reset_callback(self, msg):
        # Bump the scan window so subsequent counts only reflect boxes
        # detected from now on. Historical candidates are preserved for
        # marker display.
        self.current_scan_id += 1
        rospy.loginfo("[BoxDet3D] Scan window advanced to id=%d (kept %d candidates)",
                      self.current_scan_id, len(self.candidates))

    def costmap_callback(self, msg):
        self.global_costmap = msg

    def _has_gap_between(self, x1, y1, x2, y2):
        """Return True if the straight line from (x1,y1) to (x2,y2) passes
        through at least one lethal (occupied) cell in the global costmap.
        Used to prevent merging two same-digit candidates that actually have
        a box/wall between them. If costmap is unavailable, returns False
        (i.e., allow merging -- fall back to distance-only behaviour)."""
    ...
    def find_or_create_candidate(self, map_x, map_y, map_z, digit):
        ...
        for cand in self.candidates:
            ...
            if d < self.same_digit_radius and d < best_dist:
                # Costmap gap check: if there is an occupied cell between
                # this candidate and the new observation, they are two
                # different physical boxes even if the digit matches.
                if self._has_gap_between(cand.x, cand.y, map_x, map_y):
                    continue
                best_dist = d
                best_cand = cand
        ...
        # New candidate, tagged with the current scan window + phase
        ...
        while merged:
            ...
            for i in range(len(self.candidates)):
                ...
                for j in range(i + 1, len(self.candidates)):
                    ...
                    # Same digit within large radius -> merge, unless a
                    # costmap obstacle sits between them (two physical boxes).
                    if (ci.best_digit() == cj.best_digit()
                            and d < self.same_digit_radius
                            and not self._has_gap_between(ci.x, ci.y, cj.x, cj.y)):
    ...
    def get_fresh_counts(self):
        """Counts from confirmed candidates whose scan_id matches the current
        window. Used by upper-phase room matching so each room scan is
        isolated even though candidates are never cleared."""
        counts = {}
        for cand in self.candidates:
            if cand.scan_id != self.current_scan_id:
                continue
    ...
    def get_deduplicated_counts(self):
        """Get box counts from confirmed LOWER-phase candidates only.
        Upper-phase detections are deliberately excluded so the overlay /
        published counts / min_box always reflect the lower-phase answer.
        Upper detections still live in self.candidates for marker display."""
        counts = {}
        for cand in self.candidates:
            if cand.phase != 'lower':
                continue
    ...
    def rgbd_callback(self, rgb_msg, depth_msg):
        ...
        # In upper phase, publish only the current scan window's counts so
        # nav_test's room matching ignores historical (lower) detections.
        # In lower phase, publish the full deduped counts.
        if self.current_phase == 'upper':
            published_counts = self.get_fresh_counts()
        else:
            published_counts = counts
        self.pub_counts.publish(String(data=str(published_counts)))

        # Only publish min_box while in lower phase. Once locked (entering
        # upper), the latched topic keeps the lower-phase answer forever.
        # Publish ALL digits tied for the minimum count, comma-separated,
        # so nav_test can match any of them in upper-room scans.
        if not self.min_box_locked and counts:
            min_count = min(counts.values())
            tied = sorted(d for d, c in counts.items() if c == min_count)
            self.pub_min_box.publish(
                String(data=','.join(str(d) for d in tied)))
    ...
    def publish_markers(self):
        ...
        # DELETEALL first so that any candidate removed by merging or any
        # stale id from a previous frame disappears, then we re-ADD all
        # current ones with infinite lifetime so they persist in RViz even
        # while detection is paused (stairway phase) or after the mission
        # finishes — the user wants to manually inspect everything.
        clear = Marker()
